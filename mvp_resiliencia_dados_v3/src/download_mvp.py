...
def parse_market_file(path):
    """
    Importador tolerante para CSV/XLS/XLSX baixado manualmente da B3.
    A intenção do MVP é preservar o arquivo oficial bruto e evitar scraping.
    """
    suffix = path.suffix.lower()
    if suffix in [".xlsx", ".xls"]:
        return pd.read_excel(path)
    if suffix == ".csv":
        # tenta formatos comuns do Brasil
        for sep, enc in [(";", "utf-8"), (";", "latin1"), (",", "utf-8"), (",", "latin1")]:
            try:
                ...
                pass
    raise ValueError(f"Formato não reconhecido: {path}")
